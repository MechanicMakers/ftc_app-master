...
@Autonomous(name="Autonumose_Competition_1_Blue_1", group="Autonomous")

public class Autonumose_Competition_1_Blue_1 extends OpMode {
    ...
    public void start() {
        ...
        if (robot.LJCS.blue() > robot.LJCS.red()){
        ...
        // Stage 2 - Checking what is the color in front of us
        // if the the front is red
        else if(robot.LJCS.red() > robot.LJCS.blue()) {
        ...
        while(!(robot.BC.blue() > robot.BC.red() && robot.BC.blue() > robot.BC.green())){
            robot.autoMoveFB(0.25);
        }
        time.reset();
        if (robot.pos == "Center")
        {
            while(robot.imu.angles.firstAngle <= -90)
            {
                robot.autoTiltLR(-0.5);
            }
        }
        if (robot.pos == "Left")
        {
        ...
        while(time.milliseconds() >= 1000)
        {
            robot.autoMoveFB(-0.25); //10 - Dirve backwards
        }
        while(time.milliseconds() >= 1000)
        {
